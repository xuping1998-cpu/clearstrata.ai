-- P0-3: invoice_ai_build_context — require property_id with invoice id (defense in depth for SECURITY DEFINER).

DROP FUNCTION IF EXISTS public.invoice_ai_build_context(uuid);
...
COMMENT ON FUNCTION public.invoice_ai_build_context(uuid, uuid) IS
  'Build JSON context for AI invoice audit (Edge + service_role). Requires matching property_id.';
