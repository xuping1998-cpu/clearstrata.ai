-- Funnel: record `approved` when staff/council completes join approval (pending → approved + membership).
-- Depends on: public.property_entry_events (20260819120000_property_entry_events.sql).
-- Replaces body from 20260730120000_residents_only_unit_enter_property_by_invite.sql with one insert added.
...
BEGIN
  ...
  BEGIN
    ...
    VALUES (
      p_property_id,
      NULLIF(trim(COALESCE(jr.invite_code, '')), ''),
      NULL,
      'approved',
      v_user_id,
      p_request_id
...
COMMENT ON FUNCTION public.approve_join_request_final(uuid, uuid, text) IS
  'Final join approval: council/admin only; jsonb result; residents + property_members owner/active; join_requests approved; property_entry_events approved row.';
