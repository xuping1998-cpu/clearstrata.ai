-- Path D: persist p_invite_code into join_requests.invite_code (merge ?code= on client in propertyEntryUnified).

/*
  # Unified property entry (入楼分流) via `submit_join_request`
  ...
  - **Public branch D**, **invite-code branch C**, and **direct-invite branch A** try auto-join
    when `requested_role = owner`, application fields are complete, and bind succeeds — **no**
