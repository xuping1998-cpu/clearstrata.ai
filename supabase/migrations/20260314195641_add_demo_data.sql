...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM votes WHERE title_en = 'Install Security Cameras in Parking Garage') THEN
    ...
    VALUES ('a35ef381-2e80-425d-be09-ad1a9e829b3c', 'Install Security Cameras in Parking Garage', '在停车库安装安全摄像头', 'Proposal to install 12 security cameras throughout the underground parking garage for enhanced security', '提议在地下停车库安装12个安全摄像头以加强安全性', NOW() - INTERVAL '2 days', NOW() + INTERVAL '5 days', 20, 'active', 28, 5, 33);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM votes WHERE title_en = 'Upgrade Gym Equipment') THEN
    INSERT INTO votes (initiated_by, title_en, title_zh, description_en, description_zh, start_date, end_date, quorum_percentage, status, yes_count, no_count, total_votes)
    VALUES ('a35ef381-2e80-425d-be09-ad1a9e829b3c', 'Upgrade Gym Equipment', '升级健身房设备', 'Replace old gym equipment with new cardio machines and free weights', '用新的有氧运动机和自由重量器材替换旧的健身设备', NOW() - INTERVAL '4 days', NOW() + INTERVAL '3 days', 20, 'active', 15, 8, 23);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM votes WHERE title_en = 'Allow Pets in Building') THEN
    INSERT INTO votes (initiated_by, title_en, title_zh, description_en, description_zh, start_date, end_date, quorum_percentage, status, yes_count, no_count, total_votes)
    VALUES ('a35ef381-2e80-425d-be09-ad1a9e829b3c', 'Allow Pets in Building', '允许在大楼内养宠物', 'Proposal to allow residents to keep small pets (under 10kg) in their units', '提议允许居民在单元内饲养小型宠物（10公斤以下）', NOW() - INTERVAL '8 days', NOW() - INTERVAL '1 day', 20, 'active', 52, 18, 70);
  END IF;
END $$;
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM maintenance_requests WHERE title_en = 'Leaking Pipe in Unit 808') THEN
    ...
    VALUES ('a35ef381-2e80-425d-be09-ad1a9e829b3c', 'Leaking Pipe in Unit 808', '808单元漏水管道', 'Kitchen sink pipe is leaking, causing water damage to the cabinet', '厨房水槽管道漏水，导致橱柜受损', 500, 'submitted');
  END IF;
  ...
  IF NOT EXISTS (SELECT 1 FROM maintenance_requests WHERE title_en = 'Elevator #2 Making Noise') THEN
    ...
    VALUES ('a35ef381-2e80-425d-be09-ad1a9e829b3c', 'Elevator #2 Making Noise', '2号电梯发出噪音', 'Elevator making grinding noise when going up', '电梯上升时发出摩擦声', 2000, 1800, 'cost_approved');
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM maintenance_requests WHERE title_en = 'Broken Light in Hallway') THEN
    INSERT INTO maintenance_requests (submitted_by, title_en, title_zh, description_en, description_zh, estimated_cost, approved_cost, status)
    VALUES ('a35ef381-2e80-425d-be09-ad1a9e829b3c', 'Broken Light in Hallway', '走廊灯损坏', 'Light fixture in 8th floor hallway not working', '8楼走廊灯具不工作', 150, 120, 'completed');
  END IF;
END $$;
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM communications WHERE title_en = 'Pool Opening Hours') THEN
    INSERT INTO communications (posted_by, category, title_en, title_zh, content_en, content_zh, status, like_count)
    VALUES ('a35ef381-2e80-425d-be09-ad1a9e829b3c', 'inquiry', 'Pool Opening Hours', '游泳池开放时间', 'What are the summer opening hours for the pool? Will it be open on weekends?', '游泳池夏季开放时间是什么？周末会开放吗？', 'pending', 8);
  END IF;
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM procurement_jobs WHERE title_en = 'Lobby Renovation') THEN
    INSERT INTO procurement_jobs (posted_by, title_en, title_zh, description_en, description_zh, estimated_budget, status)
    VALUES ('a35ef381-2e80-425d-be09-ad1a9e829b3c', 'Lobby Renovation', '大堂翻新', 'Complete renovation of main lobby including flooring and lighting', '主大堂全面翻新，包括地板和照明', 120000, 'collecting_quotes');
  END IF;
  ...
  IF job1_id IS NOT NULL THEN
    IF NOT EXISTS (SELECT 1 FROM procurement_quotes WHERE job_id = job1_id AND vendor_name = 'ElevatorPro Services') THEN
      ...
      VALUES (job1_id, 'ElevatorPro Services', 'contact@elevatorpro.com', 48000, 'Quarterly maintenance with 24/7 emergency support', '季度维护，含24/7紧急支持', 'a35ef381-2e80-425d-be09-ad1a9e829b3c');
    END IF;
  ...
  IF job2_id IS NOT NULL THEN
    IF NOT EXISTS (SELECT 1 FROM procurement_quotes WHERE job_id = job2_id AND vendor_name = 'Premium Renovations') THEN
      INSERT INTO procurement_quotes (job_id, vendor_name, vendor_contact, quoted_amount, description_en, description_zh, submitted_by)
      VALUES (job2_id, 'Premium Renovations', 'sales@premiumreno.com', 115000, 'High-end materials with 2-year warranty', '高端材料，含2年保修', 'a35ef381-2e80-425d-be09-ad1a9e829b3c');
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM procurement_quotes WHERE job_id = job2_id AND vendor_name = 'Modern Interiors') THEN
      INSERT INTO procurement_quotes (job_id, vendor_name, vendor_contact, quoted_amount, description_en, description_zh, submitted_by)
      VALUES (job2_id, 'Modern Interiors', 'hello@moderninteriors.com', 118000, 'Designer finishes with 3-year warranty', '设计师级装修，含3年保修', 'a35ef381-2e80-425d-be09-ad1a9e829b3c');
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM procurement_quotes WHERE job_id = job2_id AND vendor_name = 'BuildRight Co') THEN
      INSERT INTO procurement_quotes (job_id, vendor_name, vendor_contact, quoted_amount, description_en, description_zh, submitted_by)
      VALUES (job2_id, 'BuildRight Co', 'info@buildright.com', 122000, 'Premium materials with 5-year warranty', '优质材料，含5年保修', 'a35ef381-2e80-425d-be09-ad1a9e829b3c');
    END IF;
  END IF;
END $$;
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM hiring_jobs WHERE title_en = 'Security Guard') THEN
    INSERT INTO hiring_jobs (posted_by, title_en, title_zh, description_en, description_zh, probation_months, status)
    VALUES ('a35ef381-2e80-425d-be09-ad1a9e829b3c', 'Security Guard', '保安人员', 'Night shift security guard for building entrance and parking', '夜班保安，负责大楼入口和停车场', 2, 'open');
  END IF;
