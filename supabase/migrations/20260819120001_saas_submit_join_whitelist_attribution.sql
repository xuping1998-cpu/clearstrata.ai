-- SaaS: unit_whitelist gate for roster auto-bind + invite attribution on property_members.


CREATE OR REPLACE FUNCTION public._unit_whitelist_allows_auto(
  p_property_id uuid,
  p_unit_no text
)
RETURNS boolean
...
AS $uw$
  SELECT CASE
    WHEN p_property_id IS NULL OR NULLIF(trim(p_unit_no), '') IS NULL THEN false
    WHEN NOT EXISTS (
      SELECT 1 FROM public.unit_whitelist uw WHERE uw.property_id = p_property_id
    ) THEN true
    ELSE EXISTS (
      SELECT 1 FROM public.unit_whitelist uw
      WHERE uw.property_id = p_property_id
        AND uw.is_active
        AND lower(trim(uw.unit_no)) = lower(trim(p_unit_no))
    )
  END;
$uw$;

REVOKE ALL ON FUNCTION public._unit_whitelist_allows_auto(uuid, text) FROM PUBLIC;

ALTER TABLE public.property_members
  ADD COLUMN IF NOT EXISTS join_invite_code text,
  ADD COLUMN IF NOT EXISTS join_entry_source text;

COMMENT ON COLUMN public.property_members.join_invite_code IS 'Invite/public code used when auto-joining (analytics).';
COMMENT ON COLUMN public.property_members.join_entry_source IS 'e.g. invite_auto, public_join';
...
COMMENT ON FUNCTION public.submit_join_request(
  ...
  'Unified join: invite codes, unit_whitelist gate (when table has rows), roster auto-bind, join_requests pending; PM join_invite_code attribution on auto.';
