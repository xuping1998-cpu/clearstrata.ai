-- property_members.status is text on live DB; stop comparing to member_status enum.
-- join_requests.status remains join_request_status enum (unchanged).

-- ---------------------------------------------------------------------------
-- 1) approve_join_request (wrapper)
-- ---------------------------------------------------------------------------
...
COMMENT ON FUNCTION public.approve_join_request(uuid, text, uuid) IS
  'Council/admin/manager/property_admin join approval gate; delegates to approve_join_request_final. property_members.status compared as text.';
