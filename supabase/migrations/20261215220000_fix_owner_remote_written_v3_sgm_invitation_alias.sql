-- Fix invalid reference in ON CONFLICT DO UPDATE SET: INSERT ... AS mi must use mi.*, not meeting_invitations.*.
...
BEGIN
  ...
  ON CONFLICT (meeting_id, recipient_user_id) DO UPDATE SET
    ...
    email = COALESCE(NULLIF(trim(both FROM EXCLUDED.email::text), ''), mi.email),
    delivery_channel = 'email',
    delivery_status = CASE
      WHEN mi.delivery_status IN ('sent', 'opened', 'voted') THEN mi.delivery_status
      ELSE 'pending'
    END,
    sent_at = CASE
      WHEN mi.delivery_status IN ('sent', 'opened', 'voted') THEN mi.sent_at
      ELSE NULL
    END;
