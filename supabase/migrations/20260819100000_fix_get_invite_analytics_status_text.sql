-- Fix: operator does not exist: text = join_request_status
-- Some DBs have join_requests.status as text, or mixed casts in FILTER; compare via text.
-- PropertyInviteAnalytics uses: supabase.rpc('get_invite_analytics', { p_property_id, p_since })
