/*
  # approve_join_request_final — RETURNS jsonb (structured), idempotent

  Replaces void overload. PostgREST: always inspect `data.ok`; transport `error` only on SQL failure.

  Reviewer: active `property_members` on `p_property_id` with role **admin** or **council** only.

  Success payload includes: ok, email, user_id, property_id, unit_no, residents_outcome,
  property_members_upserted, join_request_status_updated, error (null on success).

  Failure: ok=false, error=<code>, minimal other fields.

  `uniq_pending_request` already exists in 20260724130000 (property_id + lower(trim(email)) pending).
*/
