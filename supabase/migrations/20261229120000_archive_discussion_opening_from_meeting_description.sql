-- 03 discussion archive: Opening Statement from meetings.description (not legacy opening comments).
...
BEGIN
  ...
  SELECT
    trim(both FROM regexp_replace(
      regexp_replace(coalesce(m.description_zh, ''), '<!--clearstrata-written-remote[\s\S]*?-->', '', 'g'),
      '<!--clearstrata-meeting-governance[\s\S]*?-->',
      '',
      'g'
    )),
    trim(both FROM coalesce(m.description_en, ''))
  INTO v_desc_zh, v_desc_en
  FROM public.meetings m
  WHERE m.id = p_meeting_id
  LIMIT 1;

  IF coalesce(v_desc_zh, '') <> '' THEN
    v_has_opening := true;
    v_out := v_out || v_desc_zh || E'\n';
    IF coalesce(v_desc_en, '') <> '' AND v_desc_en <> v_desc_zh THEN
      v_out := v_out || E'\n' || v_desc_en || E'\n';
    END IF;
    v_out := v_out || E'\n';
  ELSIF coalesce(v_desc_en, '') <> '' THEN
    v_has_opening := true;
    v_out := v_out || v_desc_en || E'\n\n';
  END IF;
...
COMMENT ON FUNCTION public._archive_build_discussion_snapshot(uuid) IS
  'Plain-text 03 discussion snapshot: opening from meetings.description_zh/en, then public comments/replies.';
