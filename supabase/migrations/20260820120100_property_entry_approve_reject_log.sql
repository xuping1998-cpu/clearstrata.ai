-- 人工批准 / 拒绝：property_entry_events 审计
...
BEGIN
  ...
  PERFORM public._property_entry_event_silent(
    ...
    v_target_uid,
    v_target_email,
    NULLIF(trim(both from r.full_name), ''),
    NULLIF(trim(both from r.unit_number::text), ''),
    NULLIF(trim(both from coalesce(r.invite_code, '')), ''),
    'join_request_rejected',
    'rejected',
    NULLIF(trim(both from coalesce(r.review_flag, '')), ''),
    r.whitelist_matched,
    r.unit_occupied,
    p_join_request_id,
    null,
    ...
    null,
    null,
    jsonb_build_object('rejection_reason', v_reason, 'property_name', v_property_name, 'idempotent_approve', false)
  );
